...
- ✅ เพิ่มสินค้าใหม่ผ่าน Modal + อัปโหลดรูป
- ✅ **เชื่อมกับ backend จริงแล้ว** (`../fangfangshop-back`) ผ่าน `app/api.ts`
  - สินค้ามาจาก `GET /products` แล้วรวมกับแคตตาล็อกในโค้ด (ค่าจากเซิร์ฟเวอร์ชนะเมื่อชื่อตรงกัน)
  - เพิ่ม/แก้สินค้า → `POST /products`, `PATCH /products/:id`
  - `localStorage` เหลือหน้าที่แค่ cache กันจอว่างตอนต่อ API ไม่ได้
- ✅ **ล็อกอินผ่านเซิร์ฟเวอร์** — `POST /auth/login` คืน token เก็บใน `localStorage`
  (`fangfangshop-token`), หน้าล็อกอินมีปุ่มเลือกคนจาก `GET /users`,
  ตอนเปิดเว็บเช็ค token เดิมด้วย `GET /auth/me`
- ✅ **หน้าโปรไฟล์** — เปลี่ยนชื่อที่แสดง/รูปประจำตัว (`PATCH /auth/me`)
  และเปลี่ยนรหัสผ่าน (`POST /auth/change-password` — เปลี่ยนแล้วต้องล็อกอินใหม่)
- ✅ **รูปเก็บกลางที่เซิร์ฟเวอร์** — อัปรูป (ย่อแล้ว) ไป `POST /images` ได้ path `/images/<sha256>.ext`
  แล้วผูกกับสินค้า/คน ทุกเครื่องจึงเห็นรูปเดียวกัน
  - IndexedDB (`app/local-images.ts`) เหลือหน้าที่แค่ cache ตอนออฟไลน์
  - `app/migrate-images.ts` ย้ายรูปเก่าที่ค้างใน IndexedDB ขึ้น API ให้ครั้งเดียวตอนล็อกอิน
...
> ⚠️ **กับดักที่เสียเวลาไปแล้วครั้งนึง — `url` ของรูปจาก API เป็น path ไม่ใช่ URL เต็ม**
> `imageUrl`/`avatarUrl` ที่ API คืนมาหน้าตาเป็น `/images/<sha256>.png` ถ้าเอาไปใส่ `<img src>`
> ตรงๆ เบราว์เซอร์จะไปเรียกที่โดเมนของหน้าเว็บแล้วได้ 404 — **ต้องผ่าน `resolveImageUrl()`
> ใน `app/api.ts` เสมอ** (มันจะต่อ `API_BASE` ให้เฉพาะ path ที่ขึ้นต้นด้วย `/images/`
> ส่วนรูป default ใน `public/product-images/...` ปล่อยผ่านตามเดิม)
...
- ❌ สินค้าบางส่วนยังไม่ได้ตั้งราคา (`price: 0` = "รอราคา" ไม่ได้แปลว่าฟรี)
...
- **ธีมสี = โทนครีมชานม (milk tea)** — สีหลักน้ำตาลคาราเมล `#b07a4e`, พื้นครีม `#f3e7d3`
  กำหนดเป็น CSS variables ใน `app/globals.css` (`--teal` ถูกรีแมปเป็นสีคาราเมล อย่าเข้าใจผิดว่าเป็นเขียว)
  และ token ใน `app/layout.tsx` — แก้ที่สองจุดนี้ให้ตรงกันเสมอ
- **ไม่ใช้ emoji ใน UI** — เจ้าของร้านขอให้เอา emoji ออกทั้งหมด (หมวด/หัวข้อย่อย/สินค้า)
- **แบรนด์เป็นรูปแมวโทนครีม** — คอมโพเนนต์ `CatMark` (inline SVG) ใน `app/page.tsx`
  ใช้เป็นโลโก้ร้าน (เจ้าของเลี้ยงแมว) ถ้าจะเพิ่ม icon ให้คงคอนเซปต์แมว/ครีมนี้
